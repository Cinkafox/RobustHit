...
public static class ServerPackaging
{
    ...
    private static async Task BuildPlatform(PlatformReg platform, bool skipBuild, bool hybridAcz, string configuration, IPackageLogger logger)
    {
        ...
        var sw = RStopwatch.StartNew();
        {
            await using var zipFile =
                File.Open(Path.Combine("release", $"RH.Server_{platform.Rid}.zip"), FileMode.Create, FileAccess.ReadWrite);
        ...
        CancellationToken cancel)
    {
        ...
        if (hybridAcz)
        {
            inputPassCore.InjectFileFromDisk("Content.Client.zip", Path.Combine("release", "RH.Client.zip"));
        }
